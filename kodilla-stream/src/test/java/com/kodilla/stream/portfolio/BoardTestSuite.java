...
public class BoardTestSuite {
    ...
    @Test
    public void testAddTeskListFindOutdatedTasks() {
        //Given
        Board project = prepareTestData();
    ...
    @Test
    public void testShouldAddTaskListAverageWorkingOnTask() {
        //Given
        Board project = prepareTestData();
        ...
        OptionalDouble averageResult = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated().toEpochDay() - LocalDate.now().toEpochDay())
                .flatMapToLong(d -> LongStream.of(d))
                .average();
        double result = averageResult.getAsDouble();
